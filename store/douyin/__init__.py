...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    if getattr(config, 'ENABLE_DEDUP', True):
        try:
            ...
            if dedup_manager.is_video_duplicate(aweme_id):
                utils.logger.info(f"[store.douyin.update_douyin_aweme] 视频已存在，跳过: {aweme_id}")
                return

            # 添加到去重记录
            dedup_manager.add_video_id(aweme_id)
            dedup_manager.save_and_update()
        except Exception as e:
            utils.logger.warning(f"[store.douyin.update_douyin_aweme] 去重检查失败: {e}")
    ...
    try:
        ...
        log_dir = os.path.join(config.SAVE_DATA_PATH, "douyin")
        change_logger = ChangeLogger(log_dir)
        # 获取历史数据并更新视频数
        history = change_logger._load_history()
        user_id = user_info.get("sec_uid")
        if user_id and user_id in history:
            history[user_id]['videos_count'] = history[user_id].get('videos_count', 0) + 1
            change_logger._save_history(history)
    except Exception as e:
        utils.logger.warning(f"[store.douyin.update_douyin_aweme] 更新视频计数失败: {e}")
...
async def update_dy_aweme_comment(aweme_id: str, comment_item: Dict):
    ...
    if getattr(config, 'ENABLE_DEDUP', True):
        try:
            ...
            if dedup_manager.is_comment_duplicate(comment_id):
                utils.logger.info(f"[store.douyin.update_dy_aweme_comment] 评论已存在，跳过: {comment_id}")
                return

            # 添加到去重记录
            dedup_manager.add_comment_id(comment_id)
            dedup_manager.save_and_update()
        except Exception as e:
            utils.logger.warning(f"[store.douyin.update_dy_aweme_comment] 去重检查失败: {e}")
...
async def save_creator(user_id: str, creator: Dict):
    ...
    try:
        ...
        log_dir = os.path.join(config.SAVE_DATA_PATH, "douyin")
        change_logger = ChangeLogger(log_dir)
        changes = change_logger.log_crawl(
            user_id=user_id,
            user_info=local_db_item,
            video_count=0,  # 视频数会在后续更新
            comment_count=0
        )
        if changes:
            utils.logger.info(f"[store.douyin.save_creator] 数据变化已记录: {changes}")
    except Exception as e:
        utils.logger.warning(f"[store.douyin.save_creator] 记录数据变化失败: {e}")
