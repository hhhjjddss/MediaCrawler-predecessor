        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    # Sleep after each page navigation (with random jitter)
                    import random
                    jitter = random.uniform(1.0, 5.0)
                    page_sleep = config.CRAWLER_MAX_SLEEP_SEC + jitter
                    await asyncio.sleep(page_sleep)
                    utils.logger.info(f"[XiaoHongShuCrawler.search] Sleeping for {page_sleep:.1f} seconds after page {page-1}")
    ...
    async def get_note_detail_async_task(
        ...
        async with semaphore:
            try:
                ...
                # Sleep after fetching note detail (with random jitter)
                import random
                jitter = random.uniform(0.5, 3.0)
                sleep_time = config.CRAWLER_MAX_SLEEP_SEC + jitter
                await asyncio.sleep(sleep_time)
                utils.logger.info(f"[get_note_detail_async_task] Sleeping for {sleep_time:.1f} seconds after fetching note {note_id}")
