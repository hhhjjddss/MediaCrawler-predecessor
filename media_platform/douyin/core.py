    ...
    async def batch_get_note_comments(self, aweme_list: List[str]) -> None:
        ...
        try:
            ...
            for user_id in history.keys():
                # 更新评论数量（这里简化处理，实际应该统计每个视频的评论数）
                history[user_id]['comment_count'] = history[user_id].get('comment_count', 0) + len(aweme_list)
                change_logger._save_history(history)
                utils.logger.info(f"[DouYinCrawler.batch_get_note_comments] 更新评论数量: {len(aweme_list)}")
                break
        except Exception as e:
            utils.logger.warning(f"[DouYinCrawler.batch_get_note_comments] 更新评论日志失败: {e}")
        ...
        for creator_url in config.DY_CREATOR_ID_LIST:
            ...
            try:
                ...
                if user_id in history:
                    history[user_id]['videos_count'] = len(all_video_list)
                    change_logger._save_history(history)
                    utils.logger.info(f"[DouYinCrawler.get_creators_and_videos] 更新视频数量: {len(all_video_list)}")
            except Exception as e:
                utils.logger.warning(f"[DouYinCrawler.get_creators_and_videos] 更新日志失败: {e}")
